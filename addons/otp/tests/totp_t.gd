...
func run() -> bool:
    ...
    for i in times.size():
        ...
        var totp: String = totp_generator.generate_totp(test_secret.to_ascii(), current_time)
        print(format_str % [str(i), current_time, hex_T, totp])
        assert(totp == test_totps[i], "[%s] TOTP: %s != %s" % [i, totp, test_totps[i]])
    return true
