...
func run() -> bool:
    ...
    for i in 10:
        var hmac: PoolByteArray = hotp_generator.hmac_sha_1(test_secret.to_ascii(), hotp_generator.int_to_hex(i))
        ...
        assert(hmac.hex_encode() == hex_hmac_sha_1[i], "[%s] HMAC-SHA-1: %s != %s" % [i, hmac.hex_encode(), hex_hmac_sha_1[i]])
        assert(truncated == trunc_hex[i], "[%s] Hexadecimal: %s != %s" % [i, truncated, trunc_hex[i]])
        assert(decimal == trunc_dec[i], "[%s] Decimal: %s != %s" % [i, decimal, trunc_dec[i]])
        assert(t_hotp == test_hotps[i], "[%s] HOTP: %s != %s" % [i, t_hotp, test_hotps[i]])
    return true
